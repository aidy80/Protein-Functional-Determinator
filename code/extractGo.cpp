...
const string externalDrive = "My Passport";

vector<string> writeChunks{"xae", "xaf", "xag"};
vector<string> readChunks{"xaa", "xab", "xac", "xad", "xae", "xaf", "xag"};
...
vector<string> parseGo(string protLabel) {
	...
	while (!goFile.is_open() && tries >= 0) {
		goFile.open("/Volumes/" + externalDrive + "/goLabels/" + readChunks.at(tries) + "/" + protLabel);
		tries--;
	}
	if (!goFile.is_open()) {
		//cerr << "Could not find sequence " + protLabel + "\n";	
	} else {
		//cerr << "I found sequence " + protLabel + "\n";	
	}
...
vector<proteinSeq>  parsePfamGo(string pfamLabel, string goLabel) {
    string filename = "/Volumes/" + externalDrive + "pFams/" + pfamLabel;
...
void readPfamGOsequences(vector<string> &allPfam, vector<string> &allGo) {	
	ifstream relations;
	relations.open("../association_data/pfamGOscores_1to1.csv");
	if (!relations.is_open()) {
		cout << "Could find relations file\n";	
	}
	string line;
	while(getline(relations, line)) {
		vector<string> data = split(line, ',');
		allPfam.push_back(data.at(0).substr(2,5));
		allGo.push_back(data.at(1).substr(3, 7));
	}
...
int main() {
	for (string chunk : writeChunks) {
		parseGOLabels("/Volumes/My Passport/" + chunk, "/Volumes/My Passport/goLabels/" + chunk);
	}

	vector<string> allPfam;
	vector<string> allGo;

	readPfamGOsequences(allPfam, allGo);
